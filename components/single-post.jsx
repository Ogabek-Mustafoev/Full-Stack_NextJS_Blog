...
import { getSingleSimilarPost } from "../services";
import moment from "moment/moment";
import Image from "next/image";
import Link from "next/link";

export default function SinglePost({ categories, slug }) {
  const [singlePost, setSinglePost] = useState([]);

  useEffect(() => {
    getSingleSimilarPost(categories, slug).then(res => setSinglePost(res));
  }, [categories, slug]);
  ...
  return (
    <>
      {
        singlePost.map(post => (
          <div key={post.slug} className="relative h-60 lg:h-80 md:h-72 sm:h-60">
            <div
